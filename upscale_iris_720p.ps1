...
if(-not $ffmpeg -or -not (Test-Path -LiteralPath $ffmpeg)){throw "FFMPEG_PATH does not exist: $ffmpeg"}
if(-not $modelDir -or -not (Test-Path -LiteralPath $modelDir)){throw "TVAI_MODEL_DIR does not exist: $modelDir"}
if(-not $InputFile -or -not (Test-Path -LiteralPath $InputFile)){throw "InputFile does not exist: $InputFile"}
